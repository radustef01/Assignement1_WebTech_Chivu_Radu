...
describe("Testing textProcessor function", () => {
  ...
  test("dictionary parameter should contain strings", (done) => {
    expect(() => {
      app.hash(input1, dictionary0);
    }).toThrowError("Invalid dictionary format");
    done();
  });
  test("function should not modify initial input", (done) => {
    let processedInput = app.hash(input1, dictionary1);
    expect(processedInput).toBe(
      "Lorem i###m dolor sit amet consectetur adipiscing e##t"
    );
    done();
  });
  test("function should not modify initial input", (done) => {
    let unmodifiedInput = "test";
    app.hash(unmodifiedInput, dictionary2);
    expect(unmodifiedInput).toBe("test");
    done();
  });
  test("function should work for capital letter strings", (done) => {
    let processedInput = app.hash(input1, dictionary3);
    expect(processedInput).toBe(
      "L###m ipsum dolor sit amet consectetur adipiscing elit"
    );
    done();
  });
});
